...
public class DistanceTargetSelector : ITargetSelector
{
    ...
    public List<ITargetable> GetCandidates(Vector3 origin, float range, LockOnTuning tuning, Camera viewCamera, Transform self, Faction selfFaction, ITargetable currentTarget)
    {
        _results.Clear();
        ...
        int count = Physics2D.OverlapCircle(origin, range, filter, _hitBuffer);
        ...
        {
            Vector2 toCurrent = (Vector2)currentTarget.Position - (Vector2)origin;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (self != null && col.transform.root == self.root)
                continue;
            ...
            if (tuning.excludeSameTeam && selfFaction != Faction.Neutral && targetable.Faction == selfFaction)
                continue;

            if (tuning.requireLineOfSight && !HasLineOfSight(origin, targetable.Position, tuning.obstacleMask))
                continue;
            ...
            {
                Vector2 toCandidate = (Vector2)targetable.Position - (Vector2)origin;
