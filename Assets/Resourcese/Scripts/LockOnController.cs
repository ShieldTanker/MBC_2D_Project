...
public class LockOnController : MonoBehaviour
{
    ...
    [SerializeField] Faction selfFaction = Faction.Neutral;
    ...
    {
        _candidates = _selector.GetCandidates(transform.position, agentStat.LockOnRange, tuning, viewCamera, transform, selfFaction, _currentTarget);

        if (_currentTarget != null &&
            (!_currentTarget.IsLockable || !_candidates.Contains(_currentTarget)))
        {
            ClearTarget();
        }
        ...
        // 누적 입력이 임계값을 넘는 순간(=의도적인 플릭)에만 그 방향의 가까운 타겟으로 전환
        if (_currentTarget != null && _targetSwitchCooldownTimer <= 0f &&
            _aimInputAccum.magnitude >= tuning.targetSwitchInputThreshold)
        {
            TrySwitchByDirection(_aimInputAccum);
            _aimInputAccum = Vector2.zero; // 트리거 즉시 리셋 - 같은 제스처로 재트리거되지 않도록
        }
    ...
    /// <summary>
    /// 입력 방향(dirInput)과 자기 위치 기준 각도가 targetSwitchMaxAngle 이내인 후보 중
    /// 가장 가까운 대상으로 전환한다. 조건에 맞는 후보가 없으면 아무 일도 일어나지 않는다.
    /// </summary>
    void TrySwitchByDirection(Vector2 dirInput)
    {
        Vector2 dirNormalized = dirInput.normalized;
        ...
        foreach (var candidate in _candidates)
        {
            ...
            Vector2 toCandidate = (Vector2)candidate.Position - (Vector2)transform.position;
            if (toCandidate.sqrMagnitude < 0.0001f) continue;

            float angle = Vector2.Angle(dirNormalized, toCandidate.normalized);
            if (angle > tuning.targetSwitchMaxAngle) continue;

            float distSqr = toCandidate.sqrMagnitude;
        ...
        SetTarget(best);
        _targetSwitchCooldownTimer = tuning.targetSwitchCooldown;
    }
